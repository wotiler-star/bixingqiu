  ...
  render() {
    ...
    return <div className="right-content-9 right-box">
      ...
      <div className="name a">
        <h4>企业名称：</h4>
        <input type="text" ref={'qyname'}/>
      </div>
      <div className="name a">
        <h4>营业执照号：</h4>
        <input type="text" ref={'qynumber'}/>
      </div>
      <div className="pn a">
        <h4>营业执照扫描件：</h4>
      ...
      <div className="btn">
        <button onClick={() => {
          let {qyname, qynumber, sfz, name, number} = this.refs;
          if (qyname.value && qynumber.value && sfz.value && name.value && number.value == '') return alert('内容不能为空！');
          let obj = {
            "hid": this.state.id,
            "sort": sfz.value,
            "sort0": "1",
