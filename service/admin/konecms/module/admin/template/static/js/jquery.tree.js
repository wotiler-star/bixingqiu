...
$.fn.tree = function(settings){
	...
	return $(this).each(function(){
		...
		tree.find('div:eq(0)').attr('tabindex','0');
		//set all others to -1
		tree.find('div:gt(0)').attr('tabindex','-1');
		...
		tree
			...
			.bind('expand',function(event){
				var target = $(event.target) || tree.find('div[tabindex=0]');
			...
			.bind('traverseDown',function(event){
				...
				if(targetLi.is('[aria-expanded=true]')){
					target.next().find('div').eq(0).focus();
				}
				else if(targetLi.next().length) {
					targetLi.next().find('div').eq(0).focus();
				}	
				else {				
					targetLi.parents('li').next().find('div').eq(0).focus();
				}
			})
			...
			.bind('traverseUp',function(event){
				...
				if(targetLi.prev().length){ 
					if( targetLi.prev().is('[aria-expanded=true]') ){
						targetLi.prev().find('li:visible:last div').eq(0).focus();
					}
					else{
						targetLi.prev().find('div').eq(0).focus();
					}
				}
				else { 				
					targetLi.parents('li:eq(0)').find('div').eq(0).focus();
				}
			});
		...
		tree	
			...
			.keydown(function(event){	
					var target = tree.find('div[tabindex=0]');
					...
					if(event.keyCode == 37 || event.keyCode == 38 || event.keyCode == 39 || event.keyCode == 40){
						...
						if(event.keyCode == 37){ 
							...
							//try traversing to parent
								...
								target.parents('li:eq(1)').find('div').eq(0).focus();
							}	
						}						
						...
						if(event.keyCode == 39){ 
							...
							//try traversing to child
								...
								target.parents('li:eq(0)').find('li div').eq(0).focus();
							}
						}
						//if key is up arrow
						if(event.keyCode == 38){ 
					...
					else if((event.keyCode == 13 || event.keyCode == 32) && target.is('div.tree-parent')){
							target.trigger('toggle');
							//return click event false because it's a tree node (folder)
							return false;
					}
