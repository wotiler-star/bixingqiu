...
$.initProv = function(prov, city, defaultProv, defaultCity) {
    ...
    provHtml += '<option value="_" name="-1">请选择</option>';
    for(var i = 0; i < $._cityInfo.length; i++) {
        provHtml += '<option name="' + i + '"' + ((hasDefaultProv && $._cityInfo[i].n == defaultProv) ? ' selected="selected"' : '') + 'value="'+$._cityInfo[i].n+'"'+'>' + $._cityInfo[i].n + '</option>';
    }
...
$.initCities = function(provEl, cityEl, defaultCity) {
    var num=provEl.children("option:selected").attr("name")
    var hasDefaultCity = (typeof(defaultCity) != 'undefined');
    if(num != '' && parseInt(num) >= 0) {
        var cities = $._cityInfo[parseInt(num)].c;
        var cityHtml = '';

        cityHtml += '<option value="_">请选择</option>';
        ...
        cityEl.html('<option value="_">请选择</option>');
    }
};
