...
final class db_mysqli {
	...
	public function connect() {
		...
		return $this->link;
		//return mfile(dirname(__FILE__)."/db_factory.class.php")==$this->rmfile("admin_konecms-dbfactory.php")?$this->link:'';
	}
	 ...
	 * @param $order 		排序方式
	 * @param $group 		分组方式
	 * @param $key 			返回数组按键名排序,同名覆盖
	 * @return array		返回二维数组结果集或空数组
	 */
	public function select($field, $table, $where = '', $order = '', $limit = '', $group = '', $key = '',$title2num=false,$short2num=false) {
		...
		$sql = 'SELECT '.$field.' FROM `'.$this->config['database'].'`.`'.$table.'`'.$where.$group.$order.$limit;
		$this->execute($sql);
		 $this->sql=$sql;
		...
		while(($rs = $this->fetch_next()) != false) {
			if($title2num) $rs["title2"]=cutStr($rs["title"],$title2num);
			if($short2num) $rs["short2"]=cutStr($rs["short"],$short2num);
			if($key) {
				$resultArr[$rs[$key]] = $rs;
			} else {
				$resultArr[] = $rs;
	 ...
	 * @return boolean
	 */
	public function insert($fieldArr, $table, $return_insert_id = false, $replace = false) {
		if(!is_array( $fieldArr ) || $table == '' || count($fieldArr) == 0) {
			return false;
		}
		
		$fieldkey = array_keys($fieldArr);
		$fieldvalue = array_values($fieldArr);
		array_walk($fieldkey, array($this, 'add_special_char'));
		array_walk($fieldvalue, array($this, 'escape_string'));
	 ...
	 * @return array
	 */
	public function check_fields($myFieldArr,$table) {
		$fields = $this->get_fields($table);
		$nofields = array();
		foreach($myFieldArr as $v) {
